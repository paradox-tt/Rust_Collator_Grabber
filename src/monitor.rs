...
impl CollatorMonitor {
    ...
    async fn get_chain_slot_info(&self, network: Network, chain: SystemChain) -> Option<ChainSlotInfo> {
        ...
        // Get last authored block time (only if we're a collator)
        let last_block_time = if is_invulnerable || is_candidate {
            match client.get_last_authored_block_time(&collator_account).await {
                Ok(time) => time,
                Err(e) => {
                    debug!("Failed to get last block time for {}: {}", chain_name, e);
                    None
                }
            }
        } else {
