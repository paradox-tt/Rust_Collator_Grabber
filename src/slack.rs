...
impl SlackNotifier {
    ...
    pub fn with_bot_token(bot_token: String, channel: String, user_ids: Vec<String>) -> Self {
        Self {
            webhook_url: None,
    ...
    pub async fn report_disconnect(&self, chain_name: &str, error: &str) {
        ...
        if let Some((msg_ref, started_at)) = existing {
            // Update existing message with duration
            ...
            if let Some(msg_ref) = self.post_message(&message).await {
    ...
    /// Report reconnection - deletes the disconnect alert
    pub async fn report_reconnect(&self, chain_name: &str) {
        let msg_ref = {
            let mut alerts = self.disconnect_alerts.lock().unwrap();
            alerts.remove(chain_name).map(|a| a.message_ref)
        };

        if let Some(msg_ref) = msg_ref {
            self.delete_message_by_ref(&msg_ref).await;
            info!("{}: Reconnected, deleted disconnect alert", chain_name);
        }
    }
    ...
    pub async fn report_no_blocks(&self, chain_name: &str, duration_since_last: Duration) {
        ...
        let existing = {
            ...
            alerts.get(chain_name).map(|a| a.message_ref.clone())
        };

        let message = format!(
            "⏰ *{}* - No blocks authored in *{}*\nPlease check collator status.{}",
            chain_name, duration_str, self.format_user_mentions()
        );
    ...
    /// Report block authored - deletes the block production alert
    pub async fn report_block_authored(&self, chain_name: &str) {
        ...
        if let Some(msg_ref) = msg_ref {
            self.delete_message_by_ref(&msg_ref).await;
            info!("{}: Block authored, deleted block production alert", chain_name);
        }
    }
